...
module.exports = handler(async (req) => {
  ...
  switch (b.action) {
    ...
    case 'squad': {
      ...
      /* With no bench, the squad and the starting nine are the same eleven
         names, so changing the squad is what a team change now means. */
      const locked = await squadIsLocked(mid, gw);
      const same = (entry.squad || []).length === E.SQUAD_SIZE &&
                   squad.every(p => entry.squad.includes(p));
    ...
    /* Kept so an older tab cannot error out, but with no bench there is
       nothing to substitute. */
    case 'subs': {
      fail(409, 'There is no bench any more, so there is nothing to substitute.');
    }
    ...
    case 'chip': {
      const chip = b.chip;
      const valid = ['tripleCaptain', 'allOrNothing', 'gambler', 'wildcard'];
